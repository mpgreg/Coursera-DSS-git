...
rankhospital <- function(state, outcome, num = "best") {
        ...
        if(num == "best") print(stateDataOrdered[1,returnData])
        if(num == "worst") print(stateDataOrdered[nrow(stateDataOrdered),returnData])
        if(is.numeric(num)) {
                if(num > nrow(stateData)) return(NA)
                if(num > nrow(stateDataOrdered)) stop("num is int > num reporting hospitals")
                if(num > 0 && num <= nrow(stateDataOrdered)) print(stateDataOrdered[num,returnData])
        }
}